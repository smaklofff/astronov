...
import com.askme.astronov.service.FiegnClients.yandexGpt.YandexGptAccessTokenFiegnClient;
...
public class YandexAccessTokenService {
    ...
    private final YandexGptAccessTokenFiegnClient yandexAccessToken;
    private String iamToken;

    @Scheduled(fixedRate = 3200000)
    private void updateAccessToken() {
        requestSenderUtil.setFiegnClient(yandexAccessToken);
        log.info("Start getting access token for GigaChat");
        Map<String, String> response = ConverterUtil.convertToObject(
                requestSenderUtil.sendRequest(createRequestBody(), getHeadersForAccessToken()),
                new TypeReference<>() {}
        );
    ...
    // ToDo сделать норм передачу секретов
    private YandexGptAccessTokenDto createRequestBody() {
        return YandexGptAccessTokenDto.builder()
                .yandexPassportOauthToken("y0_AgAAAAAi2PtzAATuwQAAAAEWT3FSAADdvSWnMX5D16IZ8OG3h6pu8u-g8g")
                .build();
    }
}
